...
/*   Updated: 2025/05/19 00:37:07 by tbasak           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
add_entry(t_map_str *map, t_vstr entry)
{
	...
	key = vstr_subvstr(entry, 0, index_of_eq);
	value = vstr_subvstr(entry, index_of_eq + 1, MAX_LEN);
...
env_map_init(t_carr_cstr envp)
{
	...
	while (envp[++i])
	{
		entry = cstr_subvstr(envp[i], 0, MAX_LEN);
		add_entry(&map, entry);
	}
	return (map);
}
